...
public:
    ...
    int recv_buf_native()
    {
        ...
        // for level trigger, so just recv once
        int recv_len = ::recv(fd, iov[0].iov_base, iov[0].iov_len, 0);
    ...
    int send_buf_native()
    {
        ...
        // };
        // int send_len = sendmsg(fd, &msg, 0);
        // for level trigger, just send once
        int send_len = ::send(fd, iov[0].iov_base, iov[0].iov_len, 0);
        printf("Connection::send_buf, send_len: %d\n", send_len);
    ...
    int handle_read()
    {
        int recv_len = recv_buf_native();
        if  (recv_len < 0)
            return -1;
        else if (recv_len > 0)
        {
            int send_len = send_buf_native();
            if  (send_len < 0)
                return -1;
            else if  (send_len > 0)
                set_epollin(true);
